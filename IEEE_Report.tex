...
\author{
\IEEEauthorblockN{Hiten Raj Singh (Reg No: 230905152), Aakarsh Jha (Reg No: 230905248)}
\IEEEauthorblockA{
School of Computer Science and Engineering \\
Manipal Institute of Technology \\
Under the Guidance of: Anjana S., Assistant Professor - Senior Scale}
}
...
Burns et al.~\cite{burns2016borg} describe the design principles of Borg, Kubernetes' predecessor at Google, emphasizing the importance of declarative infrastructure management and self-healing through desired-state reconciliation, which is the mechanism that drives automatic pod replacement during fault tolerance testing in our experiments.
...
The backend application uses the \texttt{mendhak/http-https-echo} Docker image, which is a lightweight HTTP server that echoes request metadata including the responding pod's hostname. This enables precise tracking of request-to-pod routing. The Kubernetes Deployment was configured with 2~replicas and the following resource constraints:
...
Fig.~\ref{fig:cpu} presents the per-pod CPU utilization over time under 100 concurrent users. The behavioral differences between algorithms are most pronounced in this phase:
...
IP Hashing introduces a significant operational trade-off. Its 95.5\% CPU distribution CV makes it a poor choice for stateless microservices when client-IP diversity is low. Yet it also delivers the best P99 latency (110\,ms) and best fault recovery time (504\,ms) in our tests. Both of these results stem from the same root cause: concentrating all traffic on a single pod eliminates inter-pod routing variance and means that pod failures often affect the idle pod rather than the active one.
...
The collected data indicates that EWMA provides the most balanced performance profile for general-purpose microservices:
\begin{itemize}
